...
Сравним сначала работу сетей на сравнительно простом изображении, где все лица одного размера и в анфас (Рис.9). Здесь обе сети отработали идеально.

<img src="results_vizualization/FasterRCNN_3.png" width="500">  <img src="results_vizualization/RetinaNet_3.png" width="500">

Рис.10 Работа FasterRCNN (treshold=0.4) и RetinaNet (treshold=0.3).

Теперь рассмотрим более сложное изображение, где лица имеют очень разные размеры и разворот. На отработке FasterRCNN хорошо детектируются лица переднего плана и совсем мелкие лица заднего плана. Между ними идет явная прослойка лиц средних размеров, на которых сеть ошиблась (false negative). Возможно, это связано с распределением лиц в train датасете, которое имеет провал на соответствующих размерах.

RetinaNet отработала последнюю картинку значительно лучше, провал детектирования лиц средних размеров гораздо менее ярко выражен. Но все равно имеется много false negative и false positive. Уменьшение порога вызывает появление на простых картинках false positive.

## Возможное применение модели в бизнесе/реальных задачах

1. Первым очевидным применением сети, детектирующей лица, являются фотоаппараты и видеокамеры, чтобы выбрать точку фокусировки. На простых фотографиях с несколькими людьми, повернутыми в фас на переднем плане, обученная в проекте сеть детектирует лица практически без ошибок.

2. Выделение на изображениях паспорта участка с фото человека. Это нужно, возможно, в любых организациях, где снимается скан паспорта. 

3. Выделение областей для работы сети по распознаванию лиц. Сеть, распознающая личность человека по его лицу, получит на вход нужный участок изображения. Такие распознаватели используются на турникетах в различных организациях или в метро.

4. Применение в камерах слежения для детектирования присутствия людей в помещении. Но в этом случае лучше, конечно, использовать сеть, детектирующую человека в полный рост.

5. Подсчет людей на фото. Сеть, обученная в проекте, может использоваться с этой целью только для простых случаев, когда лица не слишком маленькие. Для сложных фото с большим количеством людей на площадях данная сеть не подходит.
